...
- a text field and a filter button that filter names. If nothing is set in text field, show all services.

## Service List
background of the section must be white.

### Overview:
- Displays a list of Services as Service Cards. Service are orderdered by last modified datetime descending (the most recently modified first).
- One card per row
...
- Service name
- Service DID
- A green badge if the service is verifiable
